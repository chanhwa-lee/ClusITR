...
## Step 3. T model (Poisson distribution)
...
mu_RT <- function(j, A, X, N){
  with(X,
       5*(1*abs(X1[j]) + 0.5*X2[j] + 
             (1*X1[j] - 0.5*X2[j])*A[j] + 
             5*1/(N-1)*( sum((X3)*A) - (X3[j])*A[j] ))
  )
}

T.gen <- function(A, X, N){
  T <- sapply(1:N, function(j) rpois(1, mu_RT(j, A, X, N)))
}

### beta.Oracle from indirect learning - by maximizing \sum_j mu_T(j,A,X,N)
beta.Oracle = c(0, 1, -0.5, 5)

## Step 4. C model (exponential distribution)
C.gen <- function(A, X, N){
  lambda_C <- with(X, -2 + 0.5*A + 0.5*X1 - 1*X2 - 0.5*X3)
