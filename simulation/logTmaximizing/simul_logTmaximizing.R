...
mu_RT <- function(j, A, X, N){
  with(X,
       (0.2*X2[j] + 0.2*X3[j] + 
          (0.5 + 1*X1[j] + 0.5*X2[j] - 1*X3[j] - 0.5*X4[j])*A[j] + 
          1.5*( sum(1/(N-1) * (X3 + X4) * A) - 1/(N-1) * (X3[j] + X4[j]) * A[j]))
  )
}

T.gen <- function(A, X, N){
  T <- sapply(1:N, function(j) exp(rnorm(1, mu_RT(j, A, X, N), 0.1)))
}

### beta.Oracle from indirect learning - by maximizing \sum_j mu_RT(j,A,X,N)
beta.Oracle = c(0.5, 1, 0.5, 0.5, 1)

## Step 4. C model (exponential distribution)
C.gen <- function(A, X, N){
  lambda_C <- with(X, -3 + 0.5*A + 0.5*X1 - 0.5*X2 + 0.5*X3)
  C <- rexp(N, exp(lambda_C))     
}

## Covariates for C model
X.C.names = c("X1", "X2", "X3")
...
A.train = function(data_train, X.A.names){
  ...
  A.fit <- glm(A ~ ., data = data_train %>% select(c("A", X.A.names)), family = "binomial")
